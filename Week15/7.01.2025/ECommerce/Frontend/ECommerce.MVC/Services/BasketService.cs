    ...
    public class BasketService : BaseService, IBasketService
    {
        ...
        public async Task AddToBasketAsync(BasketItemModel basketItemModel)
        {
            try
            {
                ...
                var result = JsonSerializer.Deserialize<ResponseModel<BasketItemModel>>(jsonString);
        ...
        public async Task<BasketModel> GetBasketAsync(string applicationUserId)
        {
            ...
            {
                var client = GetHttpClient();
                var request = new HttpRequestMessage(HttpMethod.Get,$"baskets/add{applicationUserId}");
                ...
                var jsonString = await response.Content.ReadAsStringAsync();
                var result = JsonSerializer.Deserialize<ResponseModel<BasketModel>
        ...
        Task IBasketService.RemoveFromBasketAsync(int basketId, int productId)
        {
            throw new NotImplementedException();
        }
    }
